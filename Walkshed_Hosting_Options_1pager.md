...
- **Routing lives in:** a vendor API (Mapbox today; alternatives include OpenRouteService or HERE).
...
### Option B — Self-hosted open-source routing service on the client's infrastructure
...
### Option C — Fully client-side walksheds on a pre-published static network
...
- **Static front-end** can sit on ordinary static storage (S3/object storage) or **GitHub Pages with a client-owned URL** via a DNS alias — great for a **staging/testing** site. Static hosting alone **cannot run** Option B's engine.
- **Net:** Options A and C can run on **static hosting**; Option B requires a **server/container host** in the client's environment.

### The statewide network data (Options B & C) and the refresh reality
...
- **Can they host a container/VM, or is it static-hosting only?** → Container OK: Option B. Static-only: Options A/C.
- **Is the concern *any* external dependency, or specifically *commercial cost / a particular vendor*?** → Cost/vendor only: a non-commercial API or self-host. Any external dependency: Options B/C.
- **Do they have an authoritative statewide pedestrian/road network, or do we route on OSM?** → feeds Options B/C.
