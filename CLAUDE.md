...
js/
  ...
  projects/
    ...
    transit-travelshed.js   Transit Travelshed module: 2-column popup (Settings | Results) that computes walk → wait → ride drawn transit routes/lines → walk isochrones from a clicked map origin, with at most one transfer, rendered as 1-3 banded rings. Origin is a probe pattern (an in-popup "Pick origin on map" button arms a one-shot `App.drawMode === "ts-origin"` map click, NOT an `App.points` feature — textboxes.js's `_initDrawMode` is the precedent), rendered as a single draggable-free `maplibregl.Marker`. Settings: 1-3 time budgets (min, default 15/30/blank), walk speed (mph, default 3.1), analysis day + time (`App.getEffectiveServiceBands` selects each feature's active band; no band or blank/0 frequency excludes the feature — disclosed, not an error), wait model (max initial wait min 10, boarding penalty min 1 — capped-initial/uncapped-transfer split per `docs/transit-travelshed-plan.md` Appendix A, with an info toggle), assumed stop spacing (mi, default 0.25, for routes/lines with no associated stop Points), a routes+lines checklist (annotates schedule-less features), and an Advanced hull-detail `maxEdge` (km, default 0.3). Real stops come from Points whose `attributes.associatedRoutes` reference the feature (resolved by scanning `routeIdx`/`lineIdx`, never the array index); synthetic stops are sampled otherwise. A per-stop walk-flood cache (`stopKey|networkEpoch|budgetKm`) is filled chunked-async via `App.computeWalkCostMap`, always at the full max-budget radius so one flood serves every band and re-run; `window.Travelshed.computeArrivalTimes` (js/core/travelshed.js) does the layered-flood arrival-time math on plain JSON built from that cache. Bands are pure thresholds over one flood (`Travelshed.bandNodeSets`), polygonized via `App.polygonizeNodeSet` and ring-differenced largest-first (innermost stays solid; a `turf.difference` failure falls back to the un-differenced polygon) into a 3-class Blues fill/line layer. Prompt-to-download: Calculate first checks whether the loaded network's recorded download extent (`App.getRoadDownloadExtent`) contains the analysis's required extent (origin's full-budget walk circle ∪ a full-budget-walk buffer around each selected feature); if not (or no network loaded), it offers a scoped `App.fetchRoadNetworkForExtent` download and auto re-runs on success — a file-imported network (extent unknown) gets a soft warning instead and is not blocked. Results: per-band area (mi²/km²) + node count + compute stats table, a per-route disclosure panel (band used, wait math, real vs sampled stop counts, direction/loop handling, or exclusion reason), a floating legend with dynamic per-band labels, GeoJSON export (band rings + origin + methodology metadata). Session persistence: settings-only schema v1 (origin, budgets, wait model, spacing, hull detail, `selectedRouteIds` keyed by the stable routeIdx/lineIdx counter so restore survives index shifts) — geometry is not persisted, so a restored session's export stays disabled until Re-run.
