...
**Tab 3 – Elasticity**: Service type dropdown, baseline/proposed headway inputs, frequency elasticity slider (0.1–1.0, default 0.5), **baseline uncertainty slider** (0–60%, step 5, default 25%). Uses `getActiveCDI()` (see below) so the CDI automatically reflects the selected corridor. Calculation flow: (1) compute `baseMid` using the full calibration formula `max(0, CDI×factor×length, (intercept+CDI×factor)×length)`, (2) apply `RidershipModel.applyBaselineUncertainty(baseMid, _baselineUncertaintyPct)` to get `{low, mid, high}` baseline band, (3) extract pure service multipliers via `RidershipModel.applyElasticity(1.0, params)`, (4) multiply aligned: `finalLow = baseBand.low × mult.low`, etc. Displays two result cards: "Baseline Projection (before service effects)" showing the uncertainty band, and "Projected Ridership (with service effects)" showing the final Conservative / Moderate / Optimistic range. Recalculates instantly on any input change (no API calls).

**Tab 4 – Scenarios**: 4-column side-by-side grid (A|B|C|D), each column containing identical input fields (name, service type, headway, span, speed, cost/rev-hr, service days). All 4 scenarios are visible simultaneously — no sub-tabs. Input IDs use `_0`/`_1`/`_2`/`_3` suffixes. Uses `getActiveCDI()` for the active corridor CDI. Applies baseline uncertainty band once (`applyBaselineUncertainty`), then for each scenario extracts pure service multipliers via `applyElasticity(1.0, ...)` and multiplies aligned (same pattern as Elasticity tab). "Build Scenarios" calls `RidershipModel.buildScenario()` for each and renders a comparison table with mid rows highlighted. CSV and JSON export enabled after build (exports include `baselineUncertaintyPct` for reproducibility).
